...
private enum PluginHost {
    static func main() {
        ...
        do {
            ...
            const Kite = Object.freeze({ version: '1.0', platform: 'macOS' });
            const SuperDD = Kite;
            """)
            context.evaluateScript(try String(contentsOf: entryURL, encoding: .utf8), withSourceURL: entryURL)
            if let exceptionMessage { throw NSError(domain: "KitePlugin", code: 1, userInfo: [NSLocalizedDescriptionKey: exceptionMessage]) }
            guard let resolve = context.objectForKeyedSubscript("resolve"), !resolve.isUndefined else {
                throw NSError(domain: "KitePlugin", code: 2, userInfo: [NSLocalizedDescriptionKey: "Plugin must define resolve(input)"])
            }
            let value = resolve.call(withArguments: [["url": request.input, "platform": "macOS"]])
            if let exceptionMessage { throw NSError(domain: "KitePlugin", code: 3, userInfo: [NSLocalizedDescriptionKey: exceptionMessage]) }
            let urls: [String]
