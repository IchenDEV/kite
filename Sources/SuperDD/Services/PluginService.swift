...
actor PluginService {
    ...
    init(directory: URL? = nil) {
        if let directory {
            ...
            self.directory = base.appending(path: "SuperDD/Plugins", directoryHint: .isDirectory)
        }
    }
    ...
    func install(_ item: PluginCatalogItem) async throws {
        ...
        let temporaryDirectory = FileManager.default.temporaryDirectory.appending(path: "SuperDDPlugin-\(UUID().uuidString)")
    ...
    private func run(plugin: PluginDescriptor, input: String) async throws -> [String] {
        ...
        return try await Task.detached(priority: .utility) {
            ...
            if let error = object["error"] as? String { throw NSError(domain: "SuperDDPlugin", code: 1, userInfo: [NSLocalizedDescriptionKey: error]) }
    ...
    private func resolveHost() throws -> URL {
        var candidates: [URL] = []
        if let resources = Bundle.main.resourceURL { candidates.append(resources.appending(path: "Helpers/superdd-plugin-host")) }
        if let executable = Bundle.main.executableURL { candidates.append(executable.deletingLastPathComponent().appending(path: "superdd-plugin-host")) }
        candidates.append(URL(fileURLWithPath: FileManager.default.currentDirectoryPath).appending(path: ".build/debug/superdd-plugin-host"))
        guard let host = candidates.first(where: { FileManager.default.isExecutableFile(atPath: $0.path) }) else {
            throw CocoaError(.fileNoSuchFile, userInfo: [NSLocalizedDescriptionKey: "The Super DD plugin host is missing"])
        }
        return host
    }
