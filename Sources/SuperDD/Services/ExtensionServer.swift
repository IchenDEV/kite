...
actor ExtensionRouter {
    ...
    private func mdxp(_ request: HTTPRequest) async -> HTTPResponse {
        ...
        do {
            ...
            case "motrix/initialize":
                result = [
                    "protocolVersion": "1.0",
                    "serverInfo": ["name": "Super DD", "version": "0.2.0"],
    ...
    <title>Super DD Remote</title><style>
    ...
    </style></head><body><header><h1>Super DD</h1><span id="status">Disconnected</span></header>
    ...
    <script>
    const $=s=>document.querySelector(s);let token=localStorage.superddToken||'';$('#secret').value=token;
    ...
    function connect(){token=$('#secret').value;localStorage.superddToken=token;load()}
...
actor RemoteControlServer {
    private var listener: NWListener?
    private let logger = Logger(subsystem: "com.chenli.superdd", category: "remote-api")
