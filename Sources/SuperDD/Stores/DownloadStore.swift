...
final class DownloadStore {
    ...
    func confirmRemoval(of task: DownloadTask, deleteFiles: Bool) async {
        ...
        alert.informativeText = deleteFiles
            ? "\(task.name) will be removed from Super DD and its downloaded files will be moved to the Trash."
    ...
    func checkForUpdates() async {
        ...
        do {
            ...
            updateStatus = availableUpdate.map { "Version \($0.version) is available" } ?? "Super DD is up to date"
