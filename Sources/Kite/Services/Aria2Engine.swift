...
actor Aria2Engine {
    ...
    private static func resolveExecutable() throws -> URL {
        var candidates: [URL] = []
        if let override = ProcessInfo.processInfo.environment["KITE_ARIA2_PATH"]
            ?? ProcessInfo.processInfo.environment["SUPERDD_ARIA2_PATH"] {
        ...
        throw CocoaError(.fileNoSuchFile, userInfo: [NSLocalizedDescriptionKey: "aria2-next is missing. Run script/fetch_engine.sh, then relaunch Kite."])
    }

    private static func prepareSupportDirectory() throws -> URL {
        let directory = try AppIdentity.applicationSupportDirectory()
