...
private struct MDXPClient {
    ...
    func call(_ method: String, params: [String: Any] = [:]) async throws -> Any {
        ...
        guard let http = response as? HTTPURLResponse, (200 ... 299).contains(http.statusCode) else {
            throw CLIError(message: "Remote server rejected the request. Check SUPERDD_REMOTE_URL and SUPERDD_REMOTE_SECRET.")
        }
...
private enum SuperDDCLI {
    static func main() async {
        do {
            ...
            guard let base = URL(string: environment["SUPERDD_REMOTE_URL"] ?? "http://127.0.0.1:29120/") else {
                throw CLIError(message: "SUPERDD_REMOTE_URL is invalid")
            }
            let secret = environment["SUPERDD_REMOTE_SECRET"] ?? ""
            guard !secret.isEmpty else { throw CLIError(message: "Set SUPERDD_REMOTE_SECRET to the secret shown in Super DD Settings → Network.") }
            ...
            case "add":
                guard arguments.count >= 2 else { throw CLIError(message: "Usage: superddctl add <url> [directory]") }
            ...
            case "get", "pause", "resume", "remove":
                guard arguments.count >= 2 else { throw CLIError(message: "Usage: superddctl \(command) <gid>") }
            ...
            FileHandle.standardError.write(Data("superddctl: \(error.localizedDescription)\n".utf8))
            exit(1)
        }
    }
    ...
    private static func printHelp() {
        print("""
        superddctl — control Super DD through its authenticated MDXP 1.0 endpoint
        ...
        Environment:
          SUPERDD_REMOTE_URL       Defaults to http://127.0.0.1:29120/
          SUPERDD_REMOTE_SECRET    Required Bearer token from Settings → Network
        """)
    }
}
