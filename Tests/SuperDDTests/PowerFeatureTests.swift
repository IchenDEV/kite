...
struct PowerFeatureTests {
    ...
    func torrentCreator() async throws {
        let directory = FileManager.default.temporaryDirectory.appending(path: "SuperDDTorrent-\(UUID().uuidString)")
    ...
    func geoIP() async throws {
        let url = FileManager.default.temporaryDirectory.appending(path: "SuperDDGeoIP-\(UUID().uuidString).csv")
    ...
    func resolverPlugin() async throws {
        let directory = FileManager.default.temporaryDirectory.appending(path: "SuperDDPlugins-\(UUID().uuidString)")
        defer { try? FileManager.default.removeItem(at: directory) }
